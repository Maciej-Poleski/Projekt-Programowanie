...
public class FileOperations {
	 ...
	 * @throws FileSaveException Błąd kopiowania pliku.
	 * @param file Plik do dodania.
	 * @param masterTag Miejsce, do którego należy skopiować plik.
	 */
	public void addFile(File file, MasterTag masterTag) throws IllegalArgumentException, FileSaveException{
		MasterTag tempSuperMasterTag = tempTags.getOldestAncestor(masterTag);
		//Path first = ...  // tutaj zostanie zwrócona ścieżka do tempSuperMasterTag z pliku, kto się zajmuję tym plikiem?
	 ...
	 * można utworzyć go na dowolnym dysku w dowolnym miejscu.
	 * 
	 ...
	 * @throws FileNotAccessibleException Brak dostępu do pliku.
	 * @param file Plik do usunięcia.
	 */
	public void deleteFile(FileID fileID) throws IllegalArgumentException, FileNotAccessibleException{
		if(fileID == null) throw new IllegalArgumentException();
		try {
			File file = FileInfo.getFile(fileID); //oczekiwanie na orginalną ścieżkę
	 ...
	 * 
	 * @throws IllegalArgumentException Jeżeli masterTag == null.
	 * @throws FileArgumentException Brak dostępu do pliku.
	 * @param masterTag MasterTag do usunięcia.
	 */
	public void deleteDirectory(MasterTag masterTag) throws IllegalArgumentException, FileNotAccessibleException{
		if(masterTag == null) throw new IllegalArgumentException();
		try {
			MasterTag tempSuperMasterTag = tempTags.getOldestAncestor(masterTag);
			//Path first = ...  // tutaj zostanie zwrócona ścieżka do tempSuperMasterTag z pliku, kto się zajmuje tym plikiem?
			Path second = tempTags.getPathFromMasterTag(masterTag);
			File real = new File(first.toString()+File.separator+second.toString());
			if(masterTag.toString().compareTo(tempTags.getOldestAncestor(masterTag).toString())) {
				//usuniecie z pliku zewnętrznego MastetTagu, kto się zajmuje tym plikiem???
			}
			//należy usunąć także fileID, ale do tego jest potrzebna fukcja Maćka getFilesFrom z klasy TagFilesStore
			deleteAll(real);
	 ...
	 * @throws FileNotAccessibleException Brak dostępu do pliku.
	 * @param file Plik (katalog) do usunięcia.
	 */
	private void deleteAll(File file) throws IllegalArgumentException, FileNotAccessibleException{
