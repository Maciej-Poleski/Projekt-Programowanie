	...
	public void apply(PixelData original, PixelData temp){
		...
		for(int i=0;i<mWidth;i++){
			for(int j=0;j<mHeight;j++){
				tempData[3*((mHeight-1-j)*mWidth+i)] = origData[3*(j*mWidth+i)];
				tempData[3*((mHeight-1-j)*mWidth+i)+1] = origData[3*(j*mWidth+i)+1];
				tempData[3*((mHeight-1-j)*mWidth+i)+2] = origData[3*(j*mWidth+i)+2];
			}
		}
	}
	/**
