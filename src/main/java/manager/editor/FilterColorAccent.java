...
/**
 * Filtr odpowiedzialny za tworzenie akcentu kolorystycznego filtr
 * dwu-argumentowy: arg1 - barwa wyróżniona arg2 - tolerancja Filtr pozostawia
 * kolory mieszczące sie w przedziale tolerancji bez zmian, zaś pozostałe
 * zmienia w odcienie szarości metodą luminestencji
 * 
 * @author Patryk
 */
public class FilterColorAccent implements IFilterRange {
	private final Range[] mRange = new Range[] {
			...
			new Range(0.0f, ColorConverter.mHueMaxValue / 2.0f, 10.0f,
					"Tolerancja") };
		...
		for (int i = 0; i < mWidth; i++) {
			for (int j = 0; j < mHeight; j++) {
				...
				if ((mHmin <= mH && mH <= mHmax)
						|| (mHmin <= mH - ColorConverter.mHueMaxValue && mH
								- ColorConverter.mHueMaxValue <= mHmax)
						|| (mHmin <= mH + ColorConverter.mHueMaxValue && mH
								+ ColorConverter.mHueMaxValue <= mHmax)) {
